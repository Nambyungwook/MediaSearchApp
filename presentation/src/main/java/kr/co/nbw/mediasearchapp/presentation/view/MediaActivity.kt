...
import kr.co.nbw.mediasearchapp.presentation.databinding.ActivityMediaBinding
import kr.co.nbw.mediasearchapp.presentation.mapper.toMediaEntity
...
class MediaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val mediaData: MediaEntity = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getParcelableExtra(EXTRA_MEDIA_DATA_KEY, MediaUiModel::class.java)?.toMediaEntity()
        } else {
            (intent.getParcelableExtra(EXTRA_MEDIA_DATA_KEY) as? MediaUiModel)?.toMediaEntity()
        } ?: kotlin.run {
